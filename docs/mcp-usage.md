...
列出相关条目 ID 和新建/更新/不投稿的理由；不编造材料。
若适合投稿，先给目标路径、新文件名 short-title.md（旧 ID 不变）和完整初稿
（更新旧条目则给增量补丁），供我审阅；能确定的直接填好。
未知作者留空；已知本次实际修改日期就直接填写 updated，日期不明时才留空。
来源或验证没有材料时保留空章节，缺项在稿外集中列出。
正文一般 500～1000 字即可，不为字数删条件或重复解释。
我审阅后再交付单个四反引号代码块中的最终 Markdown，以及独立的 PR 标题和描述。
正文确认不代表发布授权；必要的证据限定留在正文，流程提醒放在稿外。
