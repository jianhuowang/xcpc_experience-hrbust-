...
预期：命中 `wzj52501-0f0daa9b82cfef46`，来源仍为外部 `needs-review`。第 41 页已按原页恢复 `f(x) + g(x) ≤ cans`；第 42 页才显示 `h(x) ≥ g(x)`。第 43 页也已逐页视觉修订，分数包括 `19/45 = 1/5 + 1/6 + 1/18`，约束为 `a ≤ b ≤ 100`。若要对照自动修复标记，可另读第 22 页。第 46–48 页保留原文变量说明的歧义并加编者待核说明，不能据此声称完整算法已经验证。旧版本部署不含这批修订，验收需核对版本。
