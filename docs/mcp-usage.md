...
预期：命中 `wzj52501-0f0daa9b82cfef46`，来源仍为外部 `needs-review`。第 41 页已按原页恢复 `f(x) + g(x) ≤ cans`；第 42 页才显示 `h(x) ≥ g(x)`。第 43 页依据原字体编码恢复 `a ≤ b ≤ 100`，但分数的上下排列仍未恢复为结构化公式。不得因为字符恢复而宣称整页已审核。旧版本部署不含这批修订，验收需核对版本。
