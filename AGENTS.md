...
- 状态含义：`extracted` 仅表示已提取文本；`needs-review` 表示页图、公式或排版仍需原文核对，可用于定位，但不能把损坏或缺失的部分当作完整证明；`withheld` 表示来源或许可待核查，只有元数据与链接，不擅自获取正文绕过限制；`failed` 表示提取失败；`metadata` 是目录／许可等辅助资料。无论哪种状态都不等于算法已审核。
- 回答引用外部来源 ID、固定原文件链接和 PDF 页码／PPTX 幻灯片号／DOCX 段落或表格号；将原文结论、自行推导和待核实信息分开。公式失真时核对原页；当前不能读取原件就明确缺失，不猜符号、上下标或复杂度。
- 每份实际采用的外部材料附一行引用：`外部来源：ID | 固定原文件链接 | 定位 | 处理状态`。引用清单包括直接题解，以及用于补充约束或支持独立推导的题面；仅在“读取文件”中列出题面不算引用。固定链接逐份给出完整 URL，不用“同上”或本地文件名代替。定位使用物理页／幻灯片／段／表号，代码或 Markdown 使用原始行号／章节；无正文时写“不适用（仅元数据）”。精简回答也保留这行引用。
- 例如转录为 `h(x) [U+0015] g(x)` 且尚未核对原页时，应写“比较符损坏，方向待核对”；不能先补成确定不等式再用末尾免责声明抵消。独立推导须另标为推导，并说明成立条件，不能冒充原文恢复。
- 无法核实的符号或变量名保留转录原样并标记待核对，不列“疑为某符号／变量”的恢复候选；提出恢复候选同样需要原件证据。
