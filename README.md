...
[wzj52501 资料库](sources/wzj52501/README.md)提供固定版本的全量文件清单、[生成索引](sources/wzj52501/index.md)和按页段定位的转录。它是外部参考资料，不会自动变成协会已审核经验。公式或图片有损、来源待核实的文件均在清单中明确标记。

当前可直接浏览索引与对应转录；Skill 的外部检索规则和客户端验收由下一步 PR 接入，本 PR 不改变现有 Skill 行为。普通 Chat 知识包仍只包含协会经验。
